...
public class BIComputer {
	...
	public List<StockStatistic> computeStockStatistics(List<Stock> stocks) {
		...
		for (Stock stock : stocks){
			...
			if (stockPourcent <= 15.0 || stockPourcent >= 85.0){
				StockStatistic stockStatistic = new StockStatistic(stock.getStore(), stock.getProduct().getReference(), stockPourcent >= 85.0, stockPourcent <= 15.0, stock.getOrdered());
	...
	public List<SaleStatistic> computeSaleStatistics(List<Sale> sales, List<SaleStatistic> lastSaleStatistics) {
		...
		for (Sale sale : sales){
			...
			if (categorieStatistics.containsKey(sale.getProductCategory().getName())){
				TemporarySaleStatistic categoryStatistic = categorieStatistics.get(sale.getProductCategory().getName());
				...
				categorieStatistics.put(sale.getProductCategory().getName(), categoryStatistic);
			}
		}
